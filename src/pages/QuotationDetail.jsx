...
export default function QuotationDetail() {
  ...
  async function load() {
    ...
    const [{ data: q }, { data: qi }, { data: h }, { data: lr }] = await Promise.all([
      ...
      supabase.from('quotation_items').select('*, tsi:quotation_item_tsi(*)').eq('quotation_id', id),
    ...
    <>
      <div className="page-header">
        ...
        <div className="row" style={{ gap: 8, flexWrap: 'wrap' }}>
          {/* Proposta comercial */}
          <button className="btn btn-secondary" onClick={() => navigate(`/proposta/${id}`)}>
            📄 Ver proposta
          </button>
          {canEdit && (
      ...
      <div className="page-body stack">
                        ...
                        <table style={{ fontSize: '.8125rem' }}>
                          ...
                          <tbody>
                            {it.tsi.map(t => {
                              ...
                              const tMargin = t.unit_price > 0 ? ((Number(t.unit_price) - Number(t.unit_cost)) / Number(t.unit_price) * 100) : 0
                              return (
                                <tr key={t.id}>
        ...
        {quot.notes && (
          <div className="card">
            ...
            <div className="card-body" style={{ whiteSpace: 'pre-wrap', fontSize: '.875rem', color: 'var(--text-2)' }}>{quot.notes}</div>
          </div>
        )}
