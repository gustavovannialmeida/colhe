...
export default function QuotationNew() {
  ...
  const totals = items.reduce((acc, it) => {
    ...
    const freight  = Number(it.unit_freight) || 0
    acc.revenue    += qty * fp
    acc.cost       += qty * (Number(it.unit_cost) || 0)
    acc.freight    += qty * freight
    it.tsi.forEach(t => {
  ...
  const netMargin   = totals.revenue
    ? ((totals.revenue - totals.cost - freightTotal - commission) / totals.revenue) * 100 : 0
                ...
                <div className="form-grid form-grid-2">
                  ...
                  <div className="form-group">
                    <label className="form-label">
                      Frete extra (R$)
                      <span style={{ fontWeight: 400, color: 'var(--text-3)', marginLeft: 6, fontSize: '.75rem' }}>
                        taxa fixa ou mínimo adicional
                      </span>
                    </label>
...
function ItemRow({ item, idx, productSuggestions, tsiByType, onSetItem, onRemove, onAddTsi, onRemoveTsi, onSetTsi, showRemove }) {
  ...
  const fp     = Number(item.unit_price) * (1 - (Number(item.discount_pct) || 0) / 100)
  ...
  return (
    <div style={{ borderBottom: '1px solid var(--border-light)', padding: '14px 20px' }}>
      ...
      {/* Row 1: product name + category + unit + remove */}
      ...
      {/* Row 2: qty + cost + price + discount + frete/un + margin */}
        ...
        <div>
          {idx === 0 && (
            <ColLabel>
              Frete/un.
              {itemFreightTotal > 0 && (
                <span style={{ color: 'var(--amber-500)', fontWeight: 400, marginLeft: 4 }}>
                  = {fmtBRL(itemFreightTotal)}
                </span>
              )}
            </ColLabel>
          )}
          {idx > 0 && itemFreightTotal > 0 && (
            <div style={{ fontSize: '.7rem', color: 'var(--amber-500)', marginBottom: 4 }}>{fmtBRL(itemFreightTotal)}</div>
          )}
          <input className="form-control" type="number" min="0" step="0.01"
