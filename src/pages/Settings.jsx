...
function ClientsTab() {
  ...
  async function save() {
    setSaving(true)
    if (modal === 'new')  await supabase.from('clients').insert(form)
    if (modal === 'edit') await supabase.from('clients').update(form).eq('id', form.id)
      ...
      {modal && (
        <CrudModal title={modal === 'new' ? 'Novo cliente' : 'Editar cliente'}
          ...
          <div className="form-grid form-grid-2">
            ...
            <Field label="UF"        k="state"    form={form} setForm={setForm} />
            <div className="form-group" style={{ gridColumn: '1/-1' }}>
              <label className="form-label">Região</label>
              <input className="form-control" value={form.region || ''} onChange={e => setForm(f => ({...f, region: e.target.value}))} placeholder="Ex: Oeste Bahia" />
            </div>
          </div>
        </CrudModal>
      )}
...
function ProductsTab() {
  ...
  async function save() {
    setSaving(true)
    const payload = { ...form, base_cost: form.base_cost ? Number(form.base_cost) : null }
    if (modal === 'new')  await supabase.from('products').insert(payload)
    if (modal === 'edit') await supabase.from('products').update(payload).eq('id', form.id)
    await load(); setModal(null); setSaving(false)
  }
  ...
  return (
    <div className="stack">
      <div className="row-between">
        <span className="text-muted">{rows.length} produto{rows.length !== 1 ? 's' : ''}</span>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        {rows.length === 0
          ...
          : (
            <div className="table-wrap">
              <table>
                <thead><tr><th>Nome</th><th>Categoria</th><th>Unidade</th><th>Custo base</th><th>Status</th><th></th></tr></thead>
                <tbody>
                  {rows.map(r => (
                    <tr key={r.id}>
                      <td className="td-strong">{r.name}</td>
      ...
      {modal && (
        <CrudModal title={modal === 'new' ? 'Novo produto' : 'Editar produto'}
          ...
          <div className="form-grid form-grid-2">
            ...
            <Field label="Categoria" k="category" form={form} setForm={setForm} placeholder="Semente, Fertilizante…" />
            <div className="form-group">
              <label className="form-label">Unidade</label>
              <select className="form-control" value={form.unit || 'sc'} onChange={e => setForm(f => ({...f, unit: e.target.value}))}>
                {['sc','kg','L','cx','un','t','ha'].map(u => <option key={u} value={u}>{u}</option>)}
              </select>
            </div>
            <Field label="Custo base (R$)" k="base_cost" form={form} setForm={setForm} type="number" placeholder="Opcional" />
          </div>
        </CrudModal>
      )}
    </div>
  ...
  return (
    <div className="stack">
      <div className="alert" style={{ background: 'var(--blue-100)', color: 'var(--blue-600)' }}>
        💡 Para adicionar um novo vendedor, crie o usuário no painel do Supabase (Authentication → Users → Invite user). Ele aparecerá aqui no primeiro login.
      </div>
      <div className="card" style={{ overflow: 'hidden' }}>
        {rows.length === 0
        ...
        <CrudModal title="Editar vendedor" onClose={() => setModal(null)} onSave={save} saving={saving}>
          <div className="stack">
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={form.role || 'seller'} onChange={e => setForm(f => ({...f, role: e.target.value}))}>
